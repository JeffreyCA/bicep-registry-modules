metadata name = 'Redis Enterprise Cache'
metadata description = 'This module deploys a Redis Enterprise or Azure Manged Redis (Preview) cache.'
...
// TODO: database property: clientProtocol
@allowed([
  'Encrypted'
  'Plaintext'
])
...
@description('Optional. Redis clustering policy.')
...
@description('Optional. Specifies whether to defer upgrade of the Redis version.')
...
@description('Optional. Specifies whether to enable data replication for high availability. Used only with Balanced, ComputeOptimized, FlashOptimized, and MemoryOptimized SKUs.')
...
// END cluster properties

// https://learn.microsoft.com/azure/azure-cache-for-redis/cache-best-practices-enterprise-tiers#sharding-and-cpu-utilization
@allowed([
  2
  3
  4
...
@description('Optional. Determines the size of the cluster, used only with Enterprise and EnterpriseFlash SKUs. Valid values are (2, 4, 6, 8, 10) for Enterprise SKUs and (3, 9) for EnterpriseFlash SKUs.')
...
param skuName string = 'Enterprise_E1'
...
@description('Optional. Redis modules to enable. Restrictions may apply based on SKU and configuration. See https://aka.ms/redis/enterprise/modules')
...
param activeGeoReplication geoReplicationType?
...
var availabilityZones = (startsWith(skuName, 'Enterprise') || startsWith(skuName, 'EnterpriseFlash'))
  ? map(zones, zone => string(zone))
  : []
...
resource redisDatabase 'Microsoft.Cache/redisEnterprise/databases@2024-09-01-preview' = {
  ...
  properties: {
    ...
    geoReplication: !empty(activeGeoReplication) ? activeGeoReplication : null
    modules: modules
    port: port
...
// resource redis_accessPolicies 'Microsoft.Cache/redisEnterprise/accessPolicies@2024-04-01-preview' = [
...
resource redis_accessPolicyAssignments 'Microsoft.Cache/redisEnterprise/databases/accessPolicyAssignments@2024-09-01-preview' = [
...
resource redisEnterprise_roleAssignments 'Microsoft.Authorization/roleAssignments@2022-04-01' = [
...
output defaultPort int = redisDatabase.properties.port

@description('The principal ID of the system assigned identity.')
output systemAssignedMIPrincipalId string = redisCluster.?identity.?principalId ?? ''
...
type geoReplicationType = {
  ...
  @description('Required. List of database resources to link with this database, including its own.')
  linkedDatabases: linkedDatabaseType[]
}

type linkedDatabaseType = {
  @description('Required. Resource ID of linked database.')
  id: string
}
